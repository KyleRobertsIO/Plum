...
def test_set_start_timestamp(
    ...
    reset_err: Optional[StartTimestampResetError] = create_execution_recorder.set_start_timestamp()
    assert reset_err == None, 'expected None returned due to first time setting timestamp but got exception'
    assert create_execution_recorder.start_timestamp != None, 'expected datetime to be set on "start_timestamp" but was found to be None'
...
def test_set_end_timestamp(
    ...
    err: PrematureEndTimestampError = create_execution_recorder.set_end_timestamp()
    assert isinstance(err, PrematureEndTimestampError), 'expected PrematureEndTimestampError, to be returned due to missing "start_timestamp"'

    reset_err: Optional[StartTimestampResetError] = create_execution_recorder.set_start_timestamp()
    assert reset_err == None, 'expected None returned due to first time setting timestamp but got exception'
    
    err: Optional[PrematureEndTimestampError] = create_execution_recorder.set_end_timestamp()
    assert err == None, 'expected no PrematureEndTimestampError to be returned, due to "start_timestamp" being provided'

    reset_err: Optional[StartTimestampResetError] = create_execution_recorder.set_end_timestamp()
